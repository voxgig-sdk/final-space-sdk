...
class TestGetEndpointEntity:
    ...
    def test_should_stream(self):
        ...
        base = FinalSpaceSDK.test(seed, None)
        seen = list(base.GetEndpoint(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = FinalSpaceSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.GetEndpoint(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FINAL_SPACE_TEST_GET_ENDPOINT_ENTID JSON to run live")
        client = setup["client"]
        ...
        get_endpoint_ref01_list_result = get_endpoint_ref01_ent.list(get_endpoint_ref01_match, None)
...
def _get_endpoint_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("FINAL_SPACE_TEST_GET_ENDPOINT_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("FINAL_SPACE_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("FINAL_SPACE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FINAL_SPACE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
