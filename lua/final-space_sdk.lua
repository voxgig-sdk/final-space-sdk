...
local FinalSpaceSDK = {}
FinalSpaceSDK.__index = FinalSpaceSDK
...
FinalSpaceSDK._make_feature = _make_feature


function FinalSpaceSDK.new(options)
  local self = setmetatable({}, FinalSpaceSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FinalSpaceSDK:options_map()
...
function FinalSpaceSDK:get_utility()
  return Utility.copy(self._utility)
end


function FinalSpaceSDK:get_root_ctx()
  return self._rootctx
end


function FinalSpaceSDK:prepare(fetchargs)
...
function FinalSpaceSDK:direct(fetchargs)
...
-- by passing it.
function FinalSpaceSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FinalSpaceSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FinalSpaceSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function FinalSpaceSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FinalSpaceSDK.new(sdkopts)
  sdk.mode = "test"
