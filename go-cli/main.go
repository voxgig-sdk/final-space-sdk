// Command FinalSpace-cli is a CLI + REPL for the FinalSpace SDK,
...
const prompt = "final-space"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "character episode get_endpoint location quote"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FINAL_SPACE_APIKEY (api key), FINAL_SPACE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FINAL_SPACE_APIKEY carries the API key and
	// FINAL_SPACE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FINAL_SPACE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FINAL_SPACE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFinalSpaceSDK(opts)
