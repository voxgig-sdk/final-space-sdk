...
boru-driven command-line client **and** interactive REPL for the FinalSpace
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/final-space-cli)
make build

# 2. See usage (words, entities, env vars)
./final-space-cli --help

# 3. Provide credentials once, via the environment
export FINAL_SPACE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./final-space-cli list character
./final-space-cli load 1 character            # {id:1} shorthand
./final-space-cli load '{id:1}' character       # explicit match map
./final-space-cli list episode

# 5. Override the API base URL for a single call
FINAL_SPACE_BASE=https://api.example.com ./final-space-cli list character

# 6. No arguments -> interactive REPL
./final-space-cli
final-space> list character
final-space> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/final-space-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/final-space-cli list character
   ```
...
```sh
./final-space-cli load 1 character          # scalar shorthand for {id:1}
./final-space-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `final-space>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/final-space-cli`. |
...
(`./final-space-cli <expr>`) and interactively (the REPL), and expressions compose the
