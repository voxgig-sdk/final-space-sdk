...
//     (\`new FinalSpaceSDK(...)\` / \`FinalSpaceSDK.test(...)\` become
//     \`FinalSpaceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"episode":{"test01":{"id":"test01"}},"get_endpoint":{"test01":{"id":"test01"}},"location":{"test01":{"id":"test01"}},"quote":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FinalSpaceSDK(...)\` and any
// \`FinalSpaceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FinalSpaceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FinalSpaceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
