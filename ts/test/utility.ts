...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.FINALSPACE_TEST_LIVE ||
    'TRUE' === process.env.FINALSPACE_TEST_OVERRIDE
  ) {
  ...
  m.FINALSPACE_TEST_EXPLAIN = process.env.FINALSPACE_TEST_EXPLAIN || m.FINALSPACE_TEST_EXPLAIN
