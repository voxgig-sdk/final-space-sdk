...
[MCP](https://modelcontextprotocol.io) server exposing the FinalSpace SDK as
two agent tools — `final-space_list` and `final-space_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/final-space-mcp)
make build

# 2. Provide credentials via the environment
export FINAL_SPACE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user final-space \
  -- /absolute/path/to/final-space-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./final-space-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// final-space_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// final-space_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/final-space-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user final-space \
     -- "$PWD"/dist/*/final-space-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `final-space_list` and `final-space_load` tools now appear
   in new sessions. Ask the agent to *"list character using final-space"*
   and it calls `final-space_list` with `{"entity":"character"}`.
...
```sh
export FINAL_SPACE_APIKEY=sk_live_xxx            # API key
export FINAL_SPACE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `final-space_list` tool
...
### Call the `final-space_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `final-space_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `final-space_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FINAL_SPACE_APIKEY` | API key sent with every request. |
| `FINAL_SPACE_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

character | episode | get_endpoint | location | quote

### Smoke test via HTTP (raw JSON-RPC)

```sh
./final-space-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"final-space_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
