...
class FtCtrl extends FinalSpaceControl
{
...
class FtClient
{
    ...
    private FinalSpaceUtility $utility;

    public function __construct(FinalSpaceUtility $utility, array $options)
    {
    ...
    public function get_utility(): FinalSpaceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FinalSpaceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FinalSpaceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FinalSpaceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FinalSpaceConfig::make_feature($name);
    ...
    private static function buildUrl(FinalSpaceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FinalSpaceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FinalSpaceResponse($fetched);
        }
    ...
    private function fail(FinalSpaceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FinalSpaceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
