...
func TestGetEndpointEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GetEndpoint(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GetEndpoint(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		getEndpointRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.get_endpoint")))
...
func get_endpointBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FINAL_SPACE_TEST_GET_ENDPOINT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FINAL_SPACE_TEST_GET_ENDPOINT_ENTID"])
