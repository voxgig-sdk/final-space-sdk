...
func TestEpisodeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FINALSPACE_TEST_EPISODE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func episodeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FINALSPACE_TEST_EPISODE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FINALSPACE_TEST_EPISODE_ENTID"])
