...
import (
	...
	omni "github.com/voxgig-sdk/final-space-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FinalSpaceError); is {
				out := map[string]any{
					"name":    "FinalSpaceError",
...
func omniCtx(arg any, client *sdk.FinalSpaceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FinalSpaceSDK); is {
			client = live
