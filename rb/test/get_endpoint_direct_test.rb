...
class GetEndpointDirectTest < Minitest::Test
  def test_direct_list_get_endpoint
    setup = get_endpoint_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-get_endpoint", setup[:live] ? "live" : "unit")
    if _should_skip
...
def get_endpoint_direct_setup(mockres)
