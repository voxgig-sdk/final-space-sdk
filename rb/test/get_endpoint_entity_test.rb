...
class GetEndpointEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = FinalSpaceSDK.test(seed, nil)
    seen = base.GetEndpoint(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.GetEndpoint(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FINAL_SPACE_TEST_GET_ENDPOINT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    get_endpoint_ref01_list_result = get_endpoint_ref01_ent.list(get_endpoint_ref01_match, nil)
...
def get_endpoint_basic_setup(extra)
  ...
  entid_env_raw = ENV["FINAL_SPACE_TEST_GET_ENDPOINT_ENTID"]
