...
The Ruby SDK for the FinalSpace API — an entity-oriented client using idiomatic Ruby conventions.
...
raise a `FinalSpaceError` (a `StandardError` subclass), so wrap
...
```ruby
# list returns an Array of GetEndpoint records (raises on error).
...
```
rb/
├── FinalSpace_sdk.rb       -- Main SDK module
