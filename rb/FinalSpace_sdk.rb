...
require_relative 'FinalSpace_types'
...
class FinalSpaceSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FinalSpaceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FinalSpaceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FinalSpaceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FinalSpaceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FinalSpaceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FinalSpaceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FinalSpaceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FinalSpaceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FinalSpaceError.new(
        "graphql_error", "FinalSpaceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
