...
module FinalSpaceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FinalSpaceResponse.new({})
    ...
    if fetch_err
      ...
      response = FinalSpaceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FinalSpaceResponse.new(fetched)
    else
