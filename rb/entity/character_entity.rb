# FinalSpace SDK Character entity
...
class CharacterEntity
  ...
  def data_set(args)
    if args
      @_data = FinalSpaceHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FinalSpaceHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Character, Hash] the loaded Character; raises FinalSpaceError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FinalSpaceHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Character>, Array] the matching Character items; raises FinalSpaceError on failure
