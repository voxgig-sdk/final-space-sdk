...
class FinalSpaceNetsimFeature extends FinalSpaceBaseFeature
{
    ...
    private function _simulate(FinalSpaceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FinalSpaceContext $ctx, array $applied): void
    {
        $client = $this->client;
