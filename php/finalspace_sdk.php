...
class FinalSpaceSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FinalSpaceHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FinalSpaceHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FinalSpaceFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FinalSpaceFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FinalSpaceUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FinalSpaceHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FinalSpaceHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FinalSpaceHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FinalSpaceError($op . "_allow",
                "FinalSpaceSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FinalSpaceHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FinalSpaceError("graphql_error",
                "FinalSpaceSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetEndpoint($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_endpoint === null) {
                $this->_get_endpoint = new GetEndpointEntity($this, null);
            }
            return $this->_get_endpoint;
        }
        return new GetEndpointEntity($this, $data);
    }
