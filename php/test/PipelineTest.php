...
{
    private static function utility(): FinalSpaceUtility
    {
        return new FinalSpaceUtility();
    }
    ...
    private static function ctx(array $over = []): FinalSpaceContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FinalSpaceContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FinalSpaceOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FinalSpaceError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FinalSpaceOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FinalSpaceMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FinalSpaceOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FinalSpaceMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FinalSpaceOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FinalSpaceMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FinalSpaceSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FinalSpaceMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FinalSpaceResponse([]),
            'result' => new FinalSpaceResult([]),
        ]);
        [, $err] = FinalSpaceMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FinalSpaceSpec([]),
            'response' => null,
            'result' => new FinalSpaceResult([]),
        ]);
        [, $err] = FinalSpaceMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FinalSpaceSpec(['step' => 's']),
            'response' => new FinalSpaceResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FinalSpaceResult([]),
        ]);
        [, $err] = FinalSpaceMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FinalSpaceSpec(['step' => 's']),
            'response' => new FinalSpaceResponse(self::resp(200, ['v' => 1])),
            'result' => new FinalSpaceResult([]),
        ]);
        [, $err] = FinalSpaceMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FinalSpaceSpec(['step' => 's']),
            'response' => new FinalSpaceResponse(self::resp(200, ['v' => 2])),
            'result' => new FinalSpaceResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FinalSpaceMakeResponse::call($ctx);
    ...
    {
        $preset = new FinalSpaceResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FinalSpaceSpec([]),
            'response' => new FinalSpaceResponse([]),
            'result' => new FinalSpaceResult([]),
        ...
        [$got, $err] = FinalSpaceMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FinalSpaceResult([])]);
        [, $err] = FinalSpaceMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FinalSpaceSpec([]), 'result' => null]);
        [, $err] = FinalSpaceMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FinalSpaceSpec(['step' => 's']),
            'result' => new FinalSpaceResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FinalSpaceOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FinalSpaceMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FinalSpaceSpec(['step' => 's']),
            'result' => new FinalSpaceResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FinalSpaceOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FinalSpaceMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FinalSpaceResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FinalSpaceSpec([]),
            'result' => new FinalSpaceResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FinalSpaceMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FinalSpaceMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FinalSpaceError('boom', 'boom');
        $utility->fetcher = function (FinalSpaceContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FinalSpaceSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FinalSpaceMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FinalSpaceContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FinalSpaceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FinalSpaceResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        FinalSpaceMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FinalSpaceContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FinalSpaceSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FinalSpaceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FinalSpaceResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FinalSpaceSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FinalSpaceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FinalSpaceMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FinalSpaceMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FinalSpaceResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FinalSpaceDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FinalSpaceResult(['ok' => false])]);
        $this->expectException(FinalSpaceError::class);
        FinalSpaceDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FinalSpaceResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FinalSpaceMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FinalSpaceMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FinalSpaceBaseFeature();
        $b = new FinalSpaceBaseFeature();
        FinalSpaceFeatureAdd::call($ctx, $a);
        FinalSpaceFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FinalSpaceBaseFeature
    {
        $f = new FinalSpaceBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FinalSpaceFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FinalSpaceContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FinalSpaceSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FinalSpacePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        FinalSpacePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FinalSpaceResponse(['headers' => null]),
            'result' => new FinalSpaceResult([]),
        ]);
        FinalSpaceResultHeaders::call($ctx);
