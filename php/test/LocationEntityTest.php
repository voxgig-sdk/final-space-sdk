...
class LocationEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FINALSPACE_TEST_LOCATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function location_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("FINALSPACE_TEST_LOCATION_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "FINALSPACE_TEST_LOCATION_ENTID" => $idmap,
        "FINALSPACE_TEST_LIVE" => "FALSE",
        "FINALSPACE_TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["FINALSPACE_TEST_LOCATION_ENTID"]);
